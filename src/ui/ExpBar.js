...
export default class ExpBar extends Phaser.GameObjects.Graphics {
    ...
    increase(amount) {
        // 경험치는 0에서 m_maxExp 사이의 값을 갖도록 함.
    ...
    draw() {
        ...
        // 경험치 바와 경험치를 푸르게 그려준다.
        // 푸른 부분이 전체의 (m_currentExp / m_maxExp * 100)%를 차지하도록 그려준디.
        this.fillStyle(0x3665d5);
        let d = Math.floor(
            ((Config.width - 2 * this.BORDER) / this.m_maxExp) *
                this.m_currentExp
        );
