...
export default class TopBar extends Phaser.GameObjects.Graphics {
    constructor(scene) {
        ...
        // TopBar의 배경색, 너비, 높이, depth를 지정
        // setScrollFactor(0)을 설정하면 플레이어가 이동해도 TopBar는 항상 화면 상단에 고정
        ...
        // 잡은 몹의 수를 멤버 변수로 만들기 (초기값 0)
        this.m_mobsKilled = 0;
        // 잡은 몹의 수를 MOBS KILLED라는 문구 옆에 적는다.
        ...
        // 레벨을 멤버 변수로 만들어준다 (초기갑 1)
        this.m_level = 1;
        // 레벨을 LEVEL이라는 문구 옆에 적는다.
    ...
    // 이 함수는 몹이 한마리 죽을 떄마다 실행됨 (Mob.js 참고)
    gainMobsKilled() {
        this.m_mobsKilled += 1;
    ...
    // 이 함수는 경험치가 다 찰 때마다 실행됨 (ExpBar.js 참고)
    gainLevel() {
        this.m_level += 1;
        ...
        // 다음 레벨업을 위해 필요한 경험치가 20씩 증가한다.

        this.scene.m_expBar.m_maxExp += 40;
